...
totalweight$calc=rowSums(totalweight[,1:9])

totalweight[which(totalweight$calc==0),] = NA
totalweight[which(totalweight$`total weight`==0),] = NA
totalweight<-totalweight[complete.cases(totalweight),]

fit=lm(`total weight`~calc,data=totalweight)
summary(fit)
...
ggplot(data =totalweight, aes(x = `total weight`, y = calc)) + geom_point() + geom_smooth(method='lm') + theme_minimal() + xlab("Measured total weight [g]")+ylab("Total weight as sum of height layers [g]") +
...
fit1=lm(`0-0.5 total weight`~calc,data=layer_0_05_weight)
summary(fit1)
...
fit2=lm(`0-0.5_1_total_weight`~calc,data=layer_05_1_weight)
summary(fit2)
...
fit3=lm(`1_1.5_total_weight`~calc,data=layer_1_15_weight)
summary(fit3)
...
fit4=lm(`1.5_2_total_weight`~calc,data=layer_15_2_weight)
summary(fit4)
...
fit5=lm(`2_2.5_total_weight`~calc,data=layer_2_25_weight)
summary(fit5)
...
fit6=lm(`2.5_3_total_weight`~calc,data=layer_25_3_weight)
summary(fit6)
...
fit7=lm(`3_3.5_total_weight`~calc,data=layer_3_35_weight)
summary(fit7)
...
fit8=lm(`3.5_4_total_weight`~calc,data=layer_35_4_weight)
summary(fit8)
...
fit9=lm(`4_4.5_total_weight`~calc,data=layer_4_45_weight)
summary(fit9)
...
fit10=lm(`4.5_5_total_weight`~calc,data=layer_45_5_weight)
summary(fit10)

stargazer(fit, fit1,fit2,fit3,fit4,fit5,fit6,fit7,fit8,fit9, type = "text",out="closingerror_biomass.txt")