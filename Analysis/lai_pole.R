...
workdir="D:/Sync/_Amsterdam/11_AndrasProject/Analysis/2019Nov/main/"
setwd(workdir)

lidar_wpointsfield=read.csv("lidar_wpointsfield.csv")

#RF for recognise most important variables
rf=randomForest(x=lidar_wpointsfield[(lidar_wpointsfield$season=="leafoff"),10:44],y=lidar_wpointsfield[(lidar_wpointsfield$season=="leafoff"),46])
varImpPlot(rf)

#select first 10 most important c(25,11,12,26,16,30,13,18,33,17)
v=vifcor(lidar_wpointsfield[,c(25,11,12,26,16,30,13,18,33,17)],th=0.6)
v

# after multicollinearity check c(11,12,16,13,18,33,17)

# LM based on ecological expectation
ggplot(data =lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",], aes(x=sum_pole_contact, y=dNormalizedZPosOpenness_mean.grd)) + geom_point(aes(colour=factor(lake))) + geom_smooth(method='lm',aes(group=lake,colour=factor(lake))) + theme_minimal() + xlab("FHD_rao_pole") + ylab("FhD_rao_lidar") 

lm.fit_leafoff1=lm(dNormalizedZPosOpenness_mean.grd~sum_pole_contact,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",])
...
mlm.fit_leafoff=lm(gct_lai ~.,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(11,12,16,13,18,33,17,46)])
summary(mlm.fit_leafoff)
stargazer(mlm.fit_leafoff,type="text")

mlm.fit_leafon=lm(gct_lai ~.,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafon",c(11,12,16,13,18,33,17,46)])
summary(mlm.fit_leafon)
stargazer(mlm.fit_leafon,type="text")

mlm.fit_balaton=lm(gct_lai ~.,data=lidar_wpointsfield[lidar_wpointsfield$lake=="balaton",c(11,12,16,13,18,33,17,46)])
summary(mlm.fit_balaton)
stargazer(mlm.fit_balaton,type="text")

mlm.fit_ferto=lm(gct_lai ~.,data=lidar_wpointsfield[lidar_wpointsfield$lake=="ferto",c(11,12,16,13,18,33,17,46)])
summary(mlm.fit_ferto)
stargazer(mlm.fit_ferto,type="text")

mlm.fit_tisza=lm(gct_lai ~.,data=lidar_wpointsfield[(lidar_wpointsfield$lake=="tisza"&lidar_wpointsfield$season=="leafoff"),c(11,12,16,13,18,33,17,46)])
summary(mlm.fit_tisza)
stargazer(mlm.fit_tisza,type="text")

# GLM - Gaussian == linear regression
glm.fit_leafoff <- glm(gct_lai ~.,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(11,12,16,13,18,33,17,46)], family=Gamma())
...
rf.fit_leafoff <-  train(gct_lai~.,
                         data = lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(11,12,16,13,18,33,17,46)],
                         method = "rf",
                         metric = "RMSE",
                         importance=TRUE,
                         ntree=100,
...
rf.fit_leafon <-  train(gct_lai ~.,
                        data = lidar_wpointsfield[lidar_wpointsfield$season=="leafon",c(11,12,16,13,18,33,17,46)],
                        method = "rf",
                        metric = "RMSE",
                        importance=TRUE,
                        ntree=100,
...
rf.fit_balaton <-  train(gct_lai ~.,
                         data = lidar_wpointsfield[lidar_wpointsfield$lake=="balaton",c(11,12,16,13,18,33,17,46)],
                         method = "rf",
                         metric = "RMSE",
                         importance=TRUE,
                         ntree=100,
...
rf.fit_ferto <-  train(gct_lai ~.,
                       data = lidar_wpointsfield[lidar_wpointsfield$lake=="ferto",c(11,12,16,13,18,33,17,46)],
                       method = "rf",
                       metric = "RMSE",
                       importance=TRUE,
                       ntree=100,
...
rf.fit_tisza <-  train(gct_lai ~.,
                       data = lidar_wpointsfield[(lidar_wpointsfield$lake=="tisza"&lidar_wpointsfield$season=="leafoff"),c(11,12,16,13,18,33,17,46)],
                       method = "rf",
                       metric = "RMSE",
                       importance=TRUE,
                       ntree=100,
