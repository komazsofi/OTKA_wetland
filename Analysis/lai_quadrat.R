...
# after multicollinearity check c(11+2,12+2,16+2,13+2,18+2,33+2,17+2)
...
mlm.fit_leafoff=lm(sum_leaf_weight~.,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(11+2,12+2,16+2,13+2,18+2,33+2,17+2,9)])
summary(mlm.fit_leafoff)
stargazer(mlm.fit_leafoff,type="text")

# GLM - Gaussian == linear regression
glm.fit_leafoff <- glm(total.weight ~.,data=lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(18+2,26+2,14+2,11+2,7)], family=poisson())
...
rf.fit_leafoff <-  train(sum_leaf_weight ~.,
                         data = lidar_wpointsfield[lidar_wpointsfield$season=="leafoff",c(11+2,12+2,16+2,13+2,18+2,33+2,17+2,9)],
                         method = "rf",
                         metric = "RMSE",
                         importance=TRUE,
                         ntree=100,
